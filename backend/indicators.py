...
def monte_carlo(cur_price: float, atr: float, horizon_h: int, is_crypto: bool = True, fat_tails: bool = True) -> dict:
    ...
    if horizon_h <= 1:
        ...
        max_pct = max(1.2, atr_pct * 2.5) if is_crypto else max(0.6, atr_pct * 2.0)
    elif horizon_h <= 8:
        max_pct = max(2.0, atr_pct * 3.5) if is_crypto else max(1.0, atr_pct * 2.5)
    elif horizon_h <= 24:
        max_pct = max(4.0, atr_pct * 5.0) if is_crypto else max(2.5, atr_pct * 4.0)
    else:
        max_pct = 12.0 if is_crypto else 6.0
