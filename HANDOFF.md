...
7. **Bot ALWAYS trades; decision-first-then-critique (2026-07-04).** See Phase 1 "Always-trade" item — gates never veto in bot mode, they critique after the directional call. Prediction mode keeps NO_TRADE + projection.
8. **Metrics must count the WHOLE SESSION (2026-07-04).** Both WIN RATE and TRUE WIN% must cover every cycle since the bot started / was reset — not the last-20-trades window. Owner also suspects the two tiles sometimes show contradictory data → confirmed causes: last-20 window, `>= 0` vs `> 0` win definitions, and market-closed stocks frozen at 0.00%. One definition, one window (session-wide), shown consistently; optionally a recent-window figure alongside.
9. **Agents' intended roles clarified by owner (2026-07-04) — "supervisors, not stats readers."** The math AI should form its OWN market view (it could analyze the market with no indicators at all), then use the indicators as supporting evidence. The news AI should be an investigator: news + insiders + world events, connect dots ("CEO sold X, news hasn't picked it up yet → flag it"), not just sentiment-tag headlines. Verified current state: quant agent = indicator dump + a mechanical 3-of-5 checklist that forces NO_TRADE; news agent = DeepSeek V4 over RSS headlines pre-scored by keyword/FinBERT sentiment, output capped at ±15 adjustment, no insiders (smart_money_intel disconnected), no web scraping; decision agent = handcuffed to ML. NOBODY currently does the supervisor job. Phases 1-2 implement exactly the owner's vision.
10. **Cheap testing (2026-07-04):** owner wants to run the fixed bot on a locally hosted AI (Ollama — support already built in: `use_local`/`local_url`/`local_model`, default qwen2.5:7b, UI toggle `um_model_mode`) to avoid API costs during long tests. Caveat flagged: a 7B local model is NOT V4-class — fine for plumbing/mechanics tests, but win-rate results won't represent the real machine; the measured gauntlet should still run on V4 (cheap; shrink the asset list to cut cost). Local mode requires running the backend on the owner's machine (`start.sh`), since Railway can't reach the owner's localhost Ollama.
11. **Cross-agent visibility (2026-07-04):** the math AI must be able to see the news AI's findings and vice versa — today they work blind to each other (only the judge sees both verdicts). Add each agent's summary to the other's context in Phase 1/2.
12. **Forensic log must be dual-audience (2026-07-04):** every export readable by BOTH an AI (for post-mortem analysis) and the owner (plain, friendly language). Part of the Phase 3 export rebuild.
13. **Verified (2026-07-04): market-hours logic PARTIALLY exists** — `main.py:1325-1331`: weekend −12 and market-closed −8 are just penalty gates; the bot still predicts and trades closed markets, and the clock is hardcoded to Riyadh time (16:30–23:00 ≈ regular NYSE hours, no DST handling, no pre/after-market). Deploy 4 = replace penalty with actual skip + extended hours + proper US-market timezone.
14. **Verified (2026-07-04): the owner's interval philosophy IS partially deployed** — the cash-out block reopens in the FRESH prediction's direction (`main.py:674`), so stay-or-flip on profit already happens every interval. Missing: verified in-loss flips/whipsaw guard, catalyst interrupt, breakeven stop, partial cashout, thesis-flip exit — and the direction feeding it is still the broken 52% signal.
15. **TradingView (2026-07-04):** owner wants TradingView as the dashboard. What exists: Mode 1 inbound webhook (TV alerts → `/trade/webhook` → bot opens position, `trading_engine.py:219`) — but that bypasses the AI brain (a Pine script decides). Mode 3 (Pine indicator mirroring ULTRAMAX's signals on TV charts) is planned in `docs/TRADINGVIEW_INTEGRATION_PLAN.md`, NOT built. TV cannot host our dashboard; the realistic path is Mode 3 + keeping bot.html for controls. Webhooks need TV Pro+ plan. `WEBHOOK_SECRET` auth is designed but NOT yet implemented — must be built before any live use.
...
✅ **BUILT 2026-07-04 as "VERSION 3"** on branch `claude/ultramax-handoff-v2-2lmq5m` — owner said GO. Not yet merged to `main` (= not yet deployed). Ships as a switchable pipeline: `pipeline_version` 3 (default) or 2 (classic, byte-identical old behavior incl. R1) + **COMPARE MODE** (runs both brains per cycle, trades the active one, shadow-scores the other; scoreboard in `/autotrader/status.version_scoreboard` and on the dashboard). Also: `time_decay_hours` configurable (default 7), owner's 50/50 blend (judge 50% + ML 25/cluster 15/Bayes 10, directional), shadow runs suppress Telegram/session memory. Verified: synthetic tests for all new paths + live server boot + endpoint checks (no-key fallbacks exercised). Prediction (manual) mode still defaults to V2 — bot mode carries V3.
...
- **Always-trade, decision-first (owner directive 2026-07-04, REPLACES the earlier "skip if PQS < 3" idea):** in BOT mode the machine must trade every asset every cycle — when force-trade was off it went days without trading, which defeats the interval philosophy. New order of operations: (1) the agents make a pure directional call FIRST with no risk gates involved ("if you had to choose: up or down, and how confident, honestly"); (2) the gates/risk checks then CRITIQUE that decision (logged as evidence, may shade confidence/sizing) but NEVER veto in bot mode; (3) the bot trades the judge's direction at its honest confidence. This kills the 5-indicator/52% fallback: even a "forced" trade is now the full AI's best direction, not a coin flip. PREDICT mode keeps NO_TRADE (plus its projection line) — only the bot always trades. Agent prompts must be reworked to always output a directional lean (the quant agent's current 3-of-5 confluence rule at `quant_agent.py` ~113-120 mechanically forces NO_TRADE and must become bot-mode-aware).
...
12. (2026-07-04, later) Owner reviewed the draft agent prompts, agreed, and said **GO** ("Make it. Make all of this."). Decisions in that message: R1 post-trade reviewer deferred (cost; per-agent reasons in forensics + Phase 5 scoreboard cover it), blend = judge 50% / numbers 50% (ML 25, cluster 15, Bayes 10 — auto-tuned later in Phase 5), new build named **VERSION 3** with a dashboard switch back to V2.4 and a run-both COMPARE mode. **Phase 1 was then built and verified** — see §5 Phase 1 ✅ note for exactly what shipped. Deploy = merge the branch to main.
